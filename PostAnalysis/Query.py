...
def sort_and_rank(window_query_data):
    sorted_data = sorted(window_query_data, key=lambda window_detail: window_detail.value, reverse=True)
    rank = 1
...
def get_window_index(query):
    ...
    query_start_min = int(query_start[0])
    query_start_sec = int(query_start[1])
    query_end_min = int(query_end[0])
    query_end_sec = int(query_end[1])
...
if __name__ == "__main__":
    ...
    for window_data in window_datas:
        logging.info("index = "+str(window_index))
    ...
    query = raw_input("Please input the start time and end time: \n")
    while query != "0":
